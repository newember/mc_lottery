...
execute as @s[tag=nwbr_skin] at @s positioned ~ ~1 ~ run function newember_main_lottery:spawn/summon/item/skin
function newember_main_lottery:result/fetch_to_retrieve
scoreboard players operation @s nwbr_player_id = @s nwbr_lot_owner
execute as @s[tag=!nwbr_classic,tag=!nwbr_skin] if score @s nwbr_number matches 1.. run summon minecraft:item ~ ~2.5 ~ {Item:{id:"minecraft:diamond",Count:1}}
execute as @s[tag=!nwbr_classic,tag=!nwbr_skin] if score @s nwbr_number matches 1.. at @s positioned ~ ~2.5 ~ store result entity @e[type=minecraft:item,limit=1,sort=nearest,distance=..0.1] Item.Count int 1 run scoreboard players get @s nwbr_number
execute as @s[tag=!nwbr_classic,tag=!nwbr_skin] if score @s nwbr_number matches 1.. at @s positioned ~ ~2.5 ~ run function newember_main_lottery:steal_protect/init
execute as @s[tag=!nwbr_classic,tag=!nwbr_skin] if score @s nwbr_number matches 1.. at @s positioned ~ ~2.5 ~ run tag @e[type=minecraft:item,limit=1,sort=nearest,distance=..0.1] add nwbr_stl_tp
