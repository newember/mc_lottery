...
#Uninstall
execute as @s[scores={nwbr_lot_timer=-1},tag=!nwbr_classic,tag=!nwbr_skin] if score @p nwtgr_lot_r_btn matches 1 if score @p nwbr_player_id = @s nwbr_lot_owner run function newember_main_lottery:buttons/options_uninstall
execute as @s[scores={nwbr_lot_timer=-1},tag=!nwbr_classic,tag=!nwbr_skin] if score @p nwtgr_lot_r_btn matches 1 unless score @p nwbr_player_id = @s nwbr_lot_owner run tellraw @p {"text":"Cette lotterie n'est pas à toi... Tu serai pas en train d'essayer de tricher là par hasard ?","color":"dark_red","hoverEvent":{"action":"show_text","contents":{"text":"petit filou","color":"#7801FC"}}}
...
execute as @s[scores={nwbr_lot_timer=-1},tag=nwbr_skin] if score @p nwtgr_lot_r_btn matches 1 run function newember_main_lottery:buttons/options_uninstall
