...
execute if score out math <= @s nwbr_number at @a[distance=..40] if score @p nwbr_player_id = @s nwbr_player_id run advancement grant @p only newember_main_lottery:lotterie/missf_g_summon
execute if score out math <= @s nwbr_number positioned ^-1.89 ^1.7 ^0.07 run function newember_special_items:boss/guard_of_misfortune/init
