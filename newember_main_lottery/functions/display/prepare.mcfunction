...
execute as @s[tag=nwbr_special,tag=!nwbr_ultra] run scoreboard players operation @e[distance=..1.5,tag=nwbr_disp_lot,scores={nwbr_lot_score=-1}] nwbr_no_itm_disp %= #75 nwbr_number
...
execute as @s[tag=nwbr_special,tag=!nwbr_ultra] as @e[distance=..1.5,tag=nwbr_disp_lot,scores={nwbr_lot_score=-1}] run function newember_main_lottery:display/special/merge_item
