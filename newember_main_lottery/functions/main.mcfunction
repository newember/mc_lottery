...
#triggers
execute as @a[scores={nwtgr_lot_r_btn=1..}] at @s as @e[tag=nwbr_lot_main,distance=..100] if score @p nwbr_lot_id = @p nwbr_lot_id run function newember_main_lottery:buttons/triggers_options
...
#item tp
execute if score #modulo#100 nwbr_number matches 90 as @e[tag=nwbr_stl_tp] at @a if score @p nwbr_player_id = @s nwbr_player_id run tp @p @p
